...
for f in files:
    ...
    # Now correctly handles newlines and whitespace around the emoji
    c = re.sub(r'<div class="dropdown">\s*<button[^>]*id="user-icon"[^>]*>\s*👤\s*</button>\s*<ul class="dropdown-menu[^>]*>[\s\S]*?</ul>\s*</div>', simple_button, c)
